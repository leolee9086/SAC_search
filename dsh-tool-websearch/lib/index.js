...
const PARAMETERS = {
  ...
  properties: {
    ...
    numResults: {
      type: "number",
      description:
        "返回结果数，默认 30、最大 200。默认给得比「够用」大，是因为结果总得排成一个列表，"
        + "而排序只是尽力而为，并不代表它真的有效：只给几条时，排序稍有偏差，"
        + "本来正确的那条就被丢掉了，而且你根本不知道它存在。"
        + "宁可多给一些噪声，也不让一个不可靠的分类器替你做筛选。",
    },
    queryType: {
      type: "string",
      enum: QUERY_TYPES,
      description:
        ...
        + "general 走的是一份通用引擎名单，**不含图片素材/购物/影视这类垂直引擎**"
    ...
    engines: {
      ...
      items: { type: "string" },
      description:
        "显式点名要用的引擎(如 ['bing','zhihu'])，传了就只跑点名的这些。"
        + "它是**收窄**：只能在当前 queryType 已经选出的引擎里挑，"
        + "不会把该类型之外的引擎拉进来 —— 要换一批引擎，得先选对 queryType。"
...
const DESCRIPTION =
  ...
  + "失败数和召回与显示的差距都值得看一眼：失败多说明实际参与排序的引擎变少了"
  + "（常见原因是反爬：百度/搜狗要验证码、Google 返回 JS 壳），而剩下哪些引擎在工作，决定了你能看到什么；"
  + "召回远大于显示，说明结果被 numResults 截断了，你拿到的不是全部候选。"
  + "\n\n宽泛查询和精确查询各有取舍，用哪种按当次任务自己判断，这里不预设搭配。"
  + "宽泛查询召回面大，容易碰到你原本不知道存在的说法、叫法和来源；"
  + "代价是同主题的无关内容多，而且排序偏向「被多数引擎收录」的东西，也就是大众化、转载多的内容。"
  + "精确查询（长查询串、多个限定词、点名 engines）更容易直接命中某个具体页面或文献；"
  + "代价是通用引擎对长查询做的是宽松匹配，噪声照样会进来，而且目标一旦用的是别的措辞就会整片错过。"
  + "以上只是历史使用里的观察：换个查询、或者引擎当时的可用性不同，结论就会变，"
  + "要以当次实际返回的结果为准，随时调整。"
  + "\n\n排序（共识 × 相关性）只是为了让结果有个先后，不代表它真的有效：靠后的更可能是噪声，"
  + "但靠后的一样可能是你要找的那条，别只因为位置就把它排除掉。"
  + "同一结果被多个引擎命中时会标成「引擎A+N更多」——这个标记只说明该页面被更多引擎收录、"
  + "被抓取的概率更高，**不代表内容更可信**：转载多、SEO 做得好的页面同样会拿到高共识。";
