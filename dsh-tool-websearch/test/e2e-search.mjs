 ...
 * ## 为什么它不在 `pnpm test` 里
 *
 * 不是因为它"慢"，而是因为**它的失败没有确定含义**：
 * 它依赖真实网络和第三方站点，会因为限流、反爬、目标站点改版而偶发失败。
 * 这类测试一旦混进单元套件，红灯就无法区分"代码坏了"还是"外面抖了" ——
 * 于是**真失败也会被当成噪声忽略**，测试反而失去意义。
 *
 * 所以两者分开，各自保证一件它**能**保证的事：
 *   - `pnpm test`：逻辑契约（跑替身，结果完全确定）
 *   - 本文件：真实链路的可用性与数据洁净度（跑真的，接受偶发失败，按需运行）
 *
 * ## 它测什么
 *
