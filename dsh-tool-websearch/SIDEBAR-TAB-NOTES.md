...
点开页签正常、界面也渲染出来了。三步注册、插槽名、服务注入全对。

**但一搜索就报错**：`bundle 过旧，缺少 searchDetailed。请在插件目录运行 bun run build 后重载插件。`

## 根因：Node 的 ESM 模块缓存（不是"没构建"）

先查了磁盘：`lib/search.bundle.mjs` 里**有** `searchDetailed`（2 处），
构建时间也是新的（13:37:17，晚于源码改动）。所以不是构建问题。

真正的机制：**Node 的 ESM 模块缓存是按「模块说明符」记的**。
HMR 重载 `lib/index.js` 只重新执行这个文件本身，**不会**清掉它之前
`import` 过的子模块 —— 于是 `search.bundle.mjs` 还是老版本那个实例，
新导出的函数根本看不见。

**修法**（`e69deeb`）：给 bundle 的 import 带上文件 mtime 当说明符
（`search.bundle.mjs?v=<mtimeMs>`），6 处 import 全部改走 `bundleSpecifier()`。
以后每次 `pnpm run build` 之后都是新说明符，开发时改完 bundle **不用重启 Harness**。

## 顺带修：底部按钮挤不下了（哥的原话）

那一排已经有 Fetch Router / 会话块索引 / 自检，我这个 `🔎 元搜索` 是第四个，
四个字会把同排其他入口挤到第二行（哥的截图里"重启"掉到了下一行）。
标签缩成两字**「搜索」**（与「自检」等宽），全称保留在 `title` / `aria-label` 里，悬停可见。

## 教训（值得单独记）

`lib/` 下的文件变化 HMR **不会自动感知**（技能里写过这点），
而**子模块的缓存更不会**。所以"改完 build 了"**不等于**"跑的是新的"。
下次再遇到"明明改了却不生效"，**先怀疑模块缓存，别先怀疑构建** ——
这次差点又去重新构建一遍。
