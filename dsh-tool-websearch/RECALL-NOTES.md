...
## 当前待修：中文相关性区分度不足（下一步就做这个）

实测「设计师兼程序员怎么赚钱」，**完全无关的结果排在 2~4 名**
（BrainyQuote 名言站、法语自助出版站）。

根因在 `aggregator.ts` 的 `relevance()`：中文用 bigram 切分，长查询会被切成 9 个
bigram（设计师/计师兼/师兼程/兼程序/程序员/序员怎/员怎么/怎么赚/么赚钱），
一条相关结果通常只命中 1 个 → `rel ≈ 0.11`；而完全无关是 0。
再经过 `score *= 0.15 + 0.85 * rel`，两者只差 **0.24 vs 0.15** —— 几乎没区分度，
排序实际退化成"纯共识分"。

**修法方向**（按优先级）：
1. 相关性补一层**中文字符覆盖率**（`rel = max(bigram命中率, 字符覆盖率 × 0.6)`），
   把"相关"和"无关"真正拉开；注意过滤"的/是/在"这类高频字，否则覆盖率虚高。
2. 视效果考虑把相关性从"微调因子"提升为"**主排序键之一**"，
   或给低于阈值的结果**只在不占满名额时才显示**。
3. 顺带：`github-issues` / `npm` 这类 API 引擎对中文查询贡献的多是噪声
   （25 条无关 PR/包），但它们本身是好的代码检索源 ——
   应该靠相关性把它们压下去，而不是砍掉引擎。
